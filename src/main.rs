...
impl<T> LinkedList<T> {
    ...
    fn insert_front(&mut self, val: T) {
        if self.is_empty() {
            let new_head = Node { data: val, next: None };
            self.head = Some(Box::new(new_head));
            
        } else {
            let current_head = self.head.take();
            let new_head = Node { data: val, next: current_head };
            self.head = Some(Box::new(new_head));
        }
        self.len += 1;
    }
    ...
    fn remove_front(&mut self) -> Option<T> {
        if self.is_empty() {
            ...
            let removed_head = self.head.take().expect("Not empty so head should not be None.");
